spherical.quadrature <- function( functn, rule, lower=-1, upper=1, weighted=TRUE, ... )
{
###
...
###	spherical polynomial, weight function and interval [-1,1]
...
###
	...
	if ( weighted ) {
		...
		ff <- 
			if ( length( list( ... ) ) && length( formals( functn ) ) > 1 )
				function( x ) { functn( x, ... ) / spherical.weight( x ) }
			else
				function( x ) { functn( x ) / spherical.weight( x ) }
	}
