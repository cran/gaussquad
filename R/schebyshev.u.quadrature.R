schebyshev.u.quadrature <- function( functn, rule, lower=0, upper=1, weighted=TRUE, ... )
{
###
...
###	in the rule data frame.  The rule corresponds to an order n shifted
###	Chebyshev polynomial of the second kind, U_n(x), weight function and interval [0,1]
...
###
	...
	if ( weighted ) {
		...
		ff <- 
			if ( length( list( ... ) ) && length( formals( functn ) ) > 1 )
				function( x ) { functn( x, ... ) / schebyshev.u.weight( x ) }
			else
				function( x ) { functn( x ) / schebyshev.u.weight( x ) }
	}
	lambda <- upper - lower
