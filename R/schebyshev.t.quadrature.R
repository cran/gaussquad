schebyshev.t.quadrature <- function( functn, rule, lower=0, upper=1, weighted=TRUE, ... )
{
###
...
###	Chebyshev polynomial of the first kind, T_n(x), weight function and interval [0,1]
...
###
	...
	if ( weighted ) {
		...
		ff <- 
			if ( length( list( ... ) ) && length( formals( functn ) ) > 1 )
				function( x ) { functn( x, ... ) / schebyshev.t.weight( x ) }
			else
				function( x ) { functn( x ) / schebyshev.t.weight( x ) }
	}
	lambda <- upper - lower
